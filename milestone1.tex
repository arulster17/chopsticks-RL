...
\author{Kai Wang, Arul Mathur, Nikitha Maderamitla, Karim Barajas}
\date{November 2025}
...
\\By swapping $(a,b)$ if $a>b$, each player will have $15$ possible hands. With the turn counter, this becomes $15^n*n$ states given $n$ players. Each player can swap to at most 5 different pairs, and attack in $2*(2(n-1))=4n-4$ ways. For the 2-player case, this leads to $450$ states and between $4n-4$ and $4n+1$ actions per state.
\section{Methodology}
We will apply the Q-Learning algorithm to this problem. Each player will have their own individual Q-table, and the algorithm will be run with the previously described states and actions. Only the terminal states (where one player wins/loses) will have a reward. In the 2-player game, the winning states receive a reward of 1, and the losing states will receive a reward of -1. All other states will have a reward of 0. Additionally, a discount factor of 0.999 will be applied, which should help the model try to end games faster.
\end{document}
